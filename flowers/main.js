...
const flowers = [
    ...
    },
    {
        id: 7,
        type: "Gladiolus",
        colors: ["White", "Cream", "Yellow", "Red", "Pink", "Green", "Lavender", "Violet"],
        price: 1.85,
...
const zoneFlowers = () => {
    ...
    for (const flower of flowers) {
        if (flower.usdaZones.includes(criteria1) && flower.usdaZones.includes(criteria2)) {
            filteredFlowers.push(flower)
        }
    }
...
const orangeFlowers = () => {
    ...
    let criteria = "Orange"
    // create the parameter of category

    for (const flower of flowers) {
        if (flower.colors.includes(criteria)) {
            filteredFlowers.push(flower)
        }
    }
...
const flowersAsHTML = () => {
    ...
    for (const flower of flowers) {
        ...
        for (const colors of flower.colors) {
